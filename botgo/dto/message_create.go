...
type RichMediaMessage struct {
	EventID    string `json:"event_id,omitempty"`  // 要回复的事件id, 逻辑同MsgID（用于被动回复）
	MsgID      string `json:"msg_id,omitempty"`    // 要回复的消息id（用于被动回复，与EventID二选一）
	FileType   uint64 `json:"file_type,omitempty"` // 业务类型，图片，文件，语音，视频 文件类型，取值:1图片,2视频,3语音(目前语音只支持silk格式)
	URL        string `json:"url,omitempty"`
	SrvSendMsg bool   `json:"srv_send_msg,omitempty"` // true:直接发送(占用主动消息频次), false:仅上传获取file_info
	Content    string `json:"content,omitempty"`      // msg_type=7时需要填入空格
	FileData   string `json:"file_data,omitempty"`    // 【暂未支持】
}
...
type MessageToCreate struct {
	...
	MsgType int    `json:"msg_type,omitempty"` //消息类型: 0:文字消息, 2: md消息, 7: 富媒体
	Embed   *Embed `json:"embed,omitempty"`
	Ark     *Ark   `json:"ark,omitempty"`
	Image   string `json:"image,omitempty"`
	Media   *Media `json:"media,omitempty"` // 富媒体消息
	...
	MsgSeq           int                       `json:"msg_seq,omitempty"`   // 回复消息的序号，与 msg_id 联合使用
	Timestamp        int64                     `json:"timestamp,omitempty"` //TODO delete this
}

// Media 富媒体消息
type Media struct {
	FileInfo string `json:"file_info"` // 文件信息，由富媒体接口返回
}

// RichMediaResponse 富媒体上传响应
type RichMediaResponse struct {
	FileUUID string `json:"file_uuid"` // 文件 ID
	FileInfo string `json:"file_info"` // 文件信息，用于发消息接口的 media 字段
	TTL      int    `json:"ttl"`       // 有效期，表示剩余多少秒到期
	ID       string `json:"id"`        // 当 srv_send_msg=true 时返回的消息ID
}
