	...
	atoken.once.Do(func() {
		go func() { // 启动一个新的goroutine
			for {
				...
				// 提前60秒刷新token，确保在过期前完成切换（官方建议）
				refreshTime := tokenTTL - 60
				if refreshTime < 10 {
					refreshTime = tokenTTL / 2 // 如果token有效期太短，则在一半时间时刷新
				}
				select {
				case <-time.NewTimer(time.Duration(refreshTime) * time.Second).C: // 提前刷新
